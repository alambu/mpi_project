...
class Accounts extends CI_Controller
{
  ...
  function about_us_submit()
  {
	...
	public function setup_tution_fee_sub()
	{
		...
		$roll=$input['roll_no'];
		$reg=$input['reg_no'];
        $q=$this->db->query("select * from student_tution_fee where roll_no='$roll' AND reg_no='$reg'")->row();	
		$where=array(
		'roll_no'=>$roll,
		'reg_no'=>$reg
		);
...
public function daily_reports()
{
	...
	public function student_bill_collection()
	{
           if(isset($_POST['filter'])):
			...
			$data['info']=$this->lib->get_student_bill($roll_no);	
			 $data['paid']=$this->lib->get_student_ledger($roll_no);	
            $this->load->view('admin/header');
		   ...
		   elseif(isset($_POST['received'])):
			...
			$new_semester_due=($semester_due+$semester_fee)-$semester_fee_rec;
			$new_admission_due=$admission_due-$admission_fee_rec;
			if($new_admission_due<1):$new_admission_due=0;endif;
			...
			$voucherno=$this->lib->get_credit_voucher();
			 ...
			 $data=array(
				 ...
				 'others_fee'=>$others,
				 'semester_fee'=>$semester_fee_rec,
				 'admission_fee'=>$admission_fee_rec,
				 'semester_due'=>$new_semester_due,
				 'admission_due'=>$new_admission_due,
				 'total_received'=>$admission_fee_rec+$semester_fee_rec+$re_exam_fee+$others,
				 'remarks'=>$remarks
			 );
			   // echo "<pre>";
			   // print_r($data);exit;
			 ...
			 'entry_date'=>date('Y-m-d')
			  ...
			  if($semester_fee_rec==""):redirect('admin/student_bill_collection','location');endif;
			$in=$this->db->insert("student_ledger",$data);
			// echo $this->db->last_query();exit;
			 if($in==1):
			 $in=$this->db->insert("mpi_credit_voucher",$entry_date);
			 toast_set('success','Successfuly payment Received');
              redirect('admin/student_bill_collection');
			 endif;
		   else:
            $this->load->view('admin/header');
            $this->load->view('admin/student_bill_section/bill_collection');
            $this->load->view('admin/footer');
			endif;
...
//        var_dump($data);
	...
	{
	    $data['row'] = $this->db->select("*")->from("mpi_session")->where('status','1')->order_by('id','ASC')->get()->result(); 
		$this->load->view('admin/header');
		$this->load->view('admin/sidebar');
   ...
   if($_POST):
	...
	public function setup_tution_fee(){
		...
		$t=$this->db->query("SELECT * FROM student_tution_fee b where b.roll_no='$q->roll_no'")->row();
		echo $q->roll_no."%+**%%".$q->reg_no."%+**%%".$t->total_payable_fee."%+**%%".$t->admission_fee."%+**%%".$t->per_semester_payable."%+**%%".$t->status;
	}	
	//get semester fee
...
public function student_bil_info($filter='id>0')
   {    
   if($_POST):
   $filter=$this->input->post(null);
   endif;
        ...
        if ($input['new_password'] == $input['confirm_password']) {
            if ($input['current_password']$user->password)) {
               $this->db->where('id', $user_id)
                    ->update('users', ['password' => password_hash($input['new_password'], PASSWORD_BCRYPT)]);
                 $message = 'Password Updated';
                 $this->session->set_flashdata('success', $message);
                 toast_set('success', 'Password Updated');
